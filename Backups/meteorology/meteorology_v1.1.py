...
def radarbounds(Radar_Time, Radar_Height, Radar_Mag, Radar_TimeBounds, CloudNet_TimeBounds, CloudNet_Base):
	...
	for cloud_id, (radar_timebounds, cloudnet_timebounds) in enumerate(zip(Radar_TimeBounds, CloudNet_TimeBounds)):
		...
		CloudBaseMin = np.min(CloudNet_Base[cloudnet_timebounds[0]:cloudnet_timebounds[1]])
		CloudBaseMax = np.max(CloudNet_Base[cloudnet_timebounds[0]:cloudnet_timebounds[1]])
		CloudBase90	 = np.nanpercentile(CloudNet_Base[cloudnet_timebounds[0]:cloudnet_timebounds[1]], 60)
		...
		Base = CloudNet_Base[cloudnet_timebounds[0]:cloudnet_timebounds[1]]
		...
		# #[Step 7] Find ratio of white space with radar space to determine if correct cloud top was identified
		# Height_Range = np.arange(argnear(Radar_Height.ravel(),CloudBaseMin), argnear(Radar_Height.ravel(),CloudTop))
		
		# print("SHAPE", Radar_Mag[0].shape, Height_Range.shape, Cloud_Time.shape)
		
		# White_Space = np.sum(np.isnan(Radar_Mag[0][Height_Range][:,Cloud_Time]))
		# Radar_Space = Radar_Mag[0][Height_Range][:,Cloud_Time].size - White_Space 
		# Radar_Ratio = Radar_Space/White_Space
		
		# print("Radar_Ratio [Initial]", Radar_Ratio)
		
		# if Radar_Ratio < 0.5:
			
			# Superplotter = SPMods(1,"HEELO")
			# Superplotter.Radar_35GHz(Radar_Time, Radar_Height, Radar_Mag[0], 'Z')
			
			# Save_Location = "Test_Initial.png"
			# Superplotter._PlotSave(Save_Location)
		
			# for CloudTopLowered in xrange(200):
			
				# #Lower Cloud Top Height by a Unit Index
				# Top_Mask = np.tile(np.searchsorted(Radar_Height.ravel(), CloudTop) - CloudTopLowered, Cloud_Time.size)
				
				# print("Top_Mask", Radar_Height.ravel()[Top_Mask[0]])
				
				# #Update all Radar arrays to cut out extra data
				# for upper_limit, timestamp in zip(Top_Mask, Cloud_Time):
					# if isinstance(Radar_Mag, tuple):
						# for radar_type in xrange(len(Radar_Mag)):
							# Radar_Mag[radar_type][upper_limit:, timestamp] = np.nan
				...
				# #Re-do White space test
				# Height_Range = np.arange(argnear(Radar_Height.ravel(), CloudBaseMin), Top_Mask[0])
				# White_Space = np.sum(np.isnan(Radar_Mag[0][Height_Range][:,Cloud_Time]))
				# Radar_Space = Radar_Mag[0][Height_Range][:,Cloud_Time].size - White_Space 
				# Radar_Ratio = Radar_Space/White_Space
				
				# print("Radar_Ratio [%s] %s %s" % (CloudTopLowered, White_Space, Radar_Ratio))
				
				# if Radar_Ratio < 0.5:
					# #Lower the cloud top height again
					# continue 
				# else:
					# #Cloud Top Height is at acceptable levels, therefore, update CloudTop and exit for loop
					# CloudTop = Radar_Height[np.searchsorted(Radar_Height.ravel(), CloudTop) - CloudTopLowered]
					# break
