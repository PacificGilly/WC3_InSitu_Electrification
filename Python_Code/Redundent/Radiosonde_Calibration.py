...
class Radiosonde_Checks(object):
	...
	def __init__(self, data, calibrate, package_no, height_range, check, enforce_parity=False, linear=True, log=True):
	...
	def quality_control(self, calibrate, package_no, height_range, check, enforce_parity):
		...
		if package_no < 8:
			...
			#Remove anomalous values in the PANDORA channels
			#Cloud Sensors
			self.data[:,7][(self.data[:,7] < 1000) | (self.data[:,7] > 1150)] = np.nan
			self.data[:,8][(self.data[:,8] < 1000) | (self.data[:,8] > 1150)] = np.nan
	...
	def RH(self, method='arden-buck'):
		"""Correct the relative humidity sensor for the ice phase.
		
		Parameters
		----------
		method : str, optional
			The saturated vapour pressure equation to calculate the relative humidity for ice
			
		References
		----------
		[1] Nicoll K.A. (2010). Appendix B4. In:Coupling Between the Global Atmospheric 
			Electric Circuit and Clouds. PhD Thesis. University of Reading.
		[2] Buck, A. L. (1981), "New equations for computing vapor pressure and enhancement 
			factor", J. Appl. Meteorol., 20: 1527-1532
		[3] http://cires1.colorado.edu/~voemel/vp.html
		[4] Sonntag, D. (1994), Advancements in the field of hygrometry, Meteorol. Z., N. F., 
			3, 51-66
		[5] Wexler, A. (1976), Vapor Pressure Formulation for Water in Range 0 to 100°C. A Revision, 
			Journal of Research of the National Bureau of Standards, 80A, 775-785. 
		"""
		...
		elif method == 'arden-buck':
			#Equation using Arden-Buck equations (see [2])
			es_w = lambda T: 0.61121*np.exp((18.678-T/234.5)*(T/(257.14+T)))
			es_i = lambda T: 0.61115*np.exp((23.036-T/333.7)*(T/(279.82+T)))
		elif method == 'sonntag':
			#Equation using Sonntag equations (see [4])
			es_w = lambda T: np.log(-6096.9385 * T**-1 \
				+ 16.635794 * T**0 \
				- 2.711193E-2 * T**1 \
				+ 1.673952E-5 * T**2 \
				+ 2.433502 * np.log(T))
			es_i = lambda T: np.exp(-6024.5282 * T**-1 \
                + 24.721994 * T**0 \
				+ 1.0613868E-2 * T**1 \
				- 1.3198825E-5 * T**2 \
				- 0.49382577 * np.log(T))
		elif method == 'wexler':
			#Equation using Wexler equations (see [5])
			es_w = lambda T: np.exp(-0.29912729E4 * T**-2 \ 
				- 0.60170128E4 * T**-1 \
				+ 0.1887643854E2 * T**0 \
				- 0.28354721E-1 * T**1 \
				+ 0.17838301E-4 * T**2 \
				- 0.84150417E-9 * T**3 \
				+ 0.44412543E-12 * T**4 \  #This line was corrected from '-' to '+' following the original citation. (HV 20140819). The change makes only negligible difference
				+ 2.858487 * np.log(T)) / 100
			es_i = lambda T: np.exp(-0.58653696E4 * T**-1 \
				+ 0.2224103300E2 * T**0 \
				+ 0.13749042E-1 * T**1 \
				- 0.34031775E-4 * T**2 \
				+ 0.26967687E-7 * T**3 \
	...
	def cloud_calibration(self, package_no, check=None):
		"""Tries to de-trend the cloud sensor data to increase the changes within the data"""
		
		#Collect datasets
		if check is not None:
			Time = self.data[:,0][self.data[:,9] == check]
			Height_Cyan = self.data[:,1][self.data[:,9] == check]
			Height_IR = self.data[:,1]
			Cloud_Cyan = self.data[:,7][self.data[:,9] == check]
			Cloud_IR = self.data[:,8]
		else:
			Time = self.data[:,0]
			Height_Cyan = self.data[:,1]
			Height_IR = self.data[:,1]
			Cloud_Cyan = self.data[:,7]
			Cloud_IR = self.data[:,8]
			
		print("Time", Time.shape)
		print("Cloud_Cyan", np.unique(gu.antinan(Cloud_Cyan)).shape)
		print("Cloud_IR", np.unique(gu.antinan(Cloud_IR)).shape)
		print("Height_Cyan", Height_Cyan.shape)
		print("Height_IR", Height_IR)
		
		#Determine trend offset for each 0.5km of data
		Trend_Boundaries_Cyan = np.searchsorted(Height_Cyan, np.arange(0,15,0.5))
		Trend_Boundaries_IR = np.searchsorted(Height_IR, np.arange(0,15,0.5))
		
		print("Trend_Boundaries_Cyan", Trend_Boundaries_Cyan.shape)
		print("Trend_Boundaries_IR", Trend_Boundaries_IR.shape)
