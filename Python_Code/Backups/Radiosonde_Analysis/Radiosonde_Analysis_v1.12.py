...
class Radiosonde(EPCC_Importer, Radiosonde_Checks, SPRadiosonde, SPTephigram):
	...
	def _CloudIdentifier(self, Sensor_Package):
		...
		if self.verbose is True: 
			gu.cprint("[INFO] You are running _CloudIdentifier from the"
	...
	def CaseStudy_Specific(self):
		...
		for i, sensor in enumerate(Sensor_Package.astype(int)):
			...
			urllib.urlretrieve(url, self.Storage_Path + 'Plots/CaseStudy/AscentNo.' + str(sensor).rjust(2,"0") + '/SatelliteImagery_AscentNo.' + str(sensor).rjust(2,"0") + '_CaseStudyVersion.png')

		###############################################################
		"""[Step 3] Get surface pressure analysis"""
		...
		for i, sensor in enumerate(Sensor_Package.astype(int)):
			...
			print("filename_download", filename_download, "filename_save", filename_save)
		return
...
if __name__ == "__main__":
	...
	if not np.any(np.in1d(arg.calibrate, ['volts', 'units', 'counts', 'basic', 'raw'])): sys.exit("[Error] Radiosonde_Analysis requires the Calibrate argument to be specified with either 'raw', 'counts', 'volts' or 'units")
	...
	print("[INFO] Starting CaseStudy_Overview...")
