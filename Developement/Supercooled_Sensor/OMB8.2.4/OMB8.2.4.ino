// Oscillating Microbalance board OMB v.8.2.4
/*
  13th Feb 2018
  MWA modified to report MAX and PLL to 2 d.p.
  SWEEP runs over 0.02 Hz steps providing +-0.02 Hz for MAX
  PLL now records to nearest 0.01 Hz
  FHT still +-0.4 Hz for SWEEP definition
...
int freqong; int ampmax; // onoing frequency and store variale for max amplitude frequency
int freq1; int freq2; int freq3; int maxfreq; int fsum;
...
void freqout(int fout, int Npulses)    // generates a frequency fout (deciHz) for duration of Npulses
{
  float halfperiodms;
  float periodus;
  word widthms;
...
void InitSweep(int f1, int f2, int fstep) {  // quick run to initialise for FHT sweep
...
void Sweep(int f1, int f2, int fstep) {            // sweeps frequency from f1 to f2 in steps of fstep, and returns resonant frequency fres

  maxamp = 0;
  fres = 0;
  fsum = 0;
  for (int f = f1; f <= f2; f = f + fstep) { // set the frequency (given in hundredths of Hz, i.e. 2000 is 20Hz)
    halfperiodms = 50000.0 / (float)f;
    widthms = int (halfperiodms);
    padus = int( 1000.0 * (float)(halfperiodms - widthms) );
    ...
    if (state == LOW || HIGH) { // reads five periods of signal wave if PLL shows 'lock' status
      ...
      freq1 = (1 / per1) * 1e8; // convert to mHz
      freqong = freq1; // store ongoing frequency value prior to filter in next line
      if (freq1 > f + 50 || freq1 < f - 50) { // if measured frequency outside reasonable range, discard from this variable, freq1
        freq1 = 0;
      }
      else if (freq1 <= f + 50 || freq1 >= f - 50) { // keep latest figure of freq1 in stored variable freq2
        freq2 = freq1;
      }
    };
...
void loop() {
  ...
  Sweep(fhtfreq*10 - 20, fhtfreq*10 + 20, 2); // target sweep from FHT freq -0.4 to FHT freq +0.4
